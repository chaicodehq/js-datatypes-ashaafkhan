...
export function analyzeUPITransactions(transactions) {
  ...
  const avgTransaction = Math.round(
    validTransactions.reduce((sum,txn) => sum + txn.amount,0)/ transactionCount
  );
  ...
  const contactCount = validTransactions.reduce((acc,txn) =>{
    acc[txn.to] = (acc[txn.to] || 0) + 1;
    return acc;
  },{});

  const frequentContact = Object.keys(contactCount).reduce((a,b)=>
    contactCount[a] >= contactCount[b] ? a : b
  );
